...
export function POSView({ active }: { active?: boolean }) {
    ...
    }, [active, cart, isPaymentModalOpen, isClientModalOpen]);
    ...
    const updateQuantity = (id: number, delta: number) => {
        setCart(prev => prev.map(item => {
            if (item.producto.id === id) {
                const newQty = Math.max(0.1, item.cantidad + delta);
    ...
    const handleSearchEnter = (e: React.KeyboardEvent) => {
        if (e.key === "Enter" && search.trim()) {
            // Buscar por código exacto o barras primero (ideal para escáner)
            ...
            if (matched) {
                ...
                // Si no hay match exacto, quizás buscar por nombre?
                const partialMatch = productos.find(p => 
                    p.nombre.toLowerCase().includes(search.toLowerCase())
                );
                if (partialMatch) addToCart(partialMatch);
            }
        }
    };
                                        ...
                                        <div key={item.producto.id} className={cn(
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="text-sm text-muted-foreground font-mono">
                                                    {item.producto.codigo} • <span className={cn(isOutStock ? "text-destructive font-bold" : "")}>Disponible: {item.producto.stock}</span>
                                                </p>
                                            </div>
